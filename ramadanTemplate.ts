...
/** Front cover image prompt: only the child, Ramadan ftour at sunset; Moroccan table + fun Ramadan design; modern, non-stereotypical. */
const COVER_PROMPT = `{STYLE_INSTRUCTION} COMPOSITION: A warm, welcoming scene for a children's book. The Main Character (a child) is the ONLY person in the frame, standing or sitting in the center. TIME OF DAY: Ftour is when the sun goes down—show DUSK or SUNSET (golden hour), not daytime. Warm golden-orange or soft evening light through a window; cozy evening mood. SETTING: MODERN, CONTEMPORARY home—clean living room or kitchen, simple furniture, neutral or soft walls. Avoid over-stereotyped décor. On the SIDES of the frame, show a Moroccan ftour table: Moroccan tabla with a Moroccan soupière (soup tureen) for harira, zlafa or bowls, chbakia, dates, a glass of water, Moroccan pancakes (msemen or baghrir)—elegant and inviting, not cluttered. COVER DESIGN: Add subtle, fun Ramadan touches—e.g. a small crescent moon or star motif, tasteful Ramadan-related text or decorative lettering (e.g. "Ramadan" or "Ramadan Mubarak" in a corner or as a gentle design element). Keep it relevant and joyful without overdoing it; modern and clean overall. No other people. TEXT PLACEMENT: The title text must be placed on a CLEAN area (e.g. above or below the child). Leave natural negative space for the text. CHARACTERS: Only "The Main Character" (the child from the reference photo), facing the camera, in comfortable modest clothing. ${SINGLE_CHARACTER_RULE} LOGO PLACEMENT: Space at the bottom center for the logo. TYPOGRAPHY: Soft rounded sans-serif – warm, friendly, child-appropriate and highly readable. The text must contrast with the background. HEADLINE TEXT: [TITLE_PLACEHOLDER]`;
